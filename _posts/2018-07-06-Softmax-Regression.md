...
<div style="font-size: 150%;">
	...
	& = \exp\big\{\sum_{k=1}^{K-1}y_k\log(\phi_k) - \sum_{k=1}^{K-1}y_k\log(1-\sum_{k=1}^{K-1}\phi_k) + \log(1-\sum_{k=1}^{K-1}\phi_k)\big\big}\\
	& = \exp\big\{\sum_{k=1}^{K-1}y_k\log(\frac{\phi_k}{1-\sum_{k=1}^{K-1}\phi_k})} \\
	\end{align}
	$$
</div>
